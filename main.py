...
# 1. 自动检测并安装 customtkinter（彻底解决 VS Code 解释器缺少依赖的问题）
try:
    import customtkinter as ctk
    import tkinter as tk
    from tkinter import filedialog, messagebox
except ModuleNotFoundError:
    print("正在为当前 Python 环境自动配置 customtkinter 依赖，请稍候...")
...
class FreeDictService:
    ...
    def lookup(cls, word):
        ...
        return {
            ...
            "example": f"This is an illustrative sentence with '{word_clean}'."
        }
...
class AIService:
    ...
    def generate_vocab_story(cls, words, extra_count, length_target, difficulty, api_key, api_url, model_name):
        ...
        extra_instruction = ""
        if extra_count > 0:
            extra_instruction = f"4. 随机未学词拓展：请在故事中额外自然引入 {extra_count} 个地道实用的中高级新单词（用 〖*word*〗 格式标出），帮助读者拓展生词。"
